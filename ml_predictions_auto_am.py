...
# =============================================================================
    ...
    df_clim_proj_shift = reshape_shift(DS_clim_ext_projections_am_det)
    df_clim_proj_shift_12 = reshape_shift(DS_clim_ext_projections_am_det, shift_time = 12)
    # Combine both dataframes 
    df_clim_proj_shift_twoyears = df_clim_proj_shift.dropna().join(df_clim_proj_shift_12)
    ### Join planting dates
    df_clim_proj_shift = df_clim_proj_shift_twoyears.dropna().join(df_calendar_month_am)
    ...
    for feature in list(DS_clim_ext_projections_am_det.keys()):
        df_feature_proj_reshape_shift_var = pd.concat([df_clim_proj_shift.loc[:,'plant'],
                                                  df_clim_proj_shift.filter(like=feature)], axis = 1)
        
        # Shift accoording to month indicator (hence +1) - SLOW
        print(f'Shifting months per grid for run: {model} and {rcp_scenario}')
        df_feature_proj_reshape_shift_var = (df_feature_proj_reshape_shift_var.apply(lambda x : x.shift(-(int(x['plant']))+1) , axis=1)
                                    .drop(columns=['plant']))
        
        list_features_proj_reshape_shift.append(df_feature_proj_reshape_shift_var)
...
# =============================================================================
    ...
    DS_feature_proj_6mon_am_det = detrend_dataset(DS_feature_proj_6mon_am, deg = 'free', mean_data = DS_feature_season_6mon_am_det ) 
...
# =============================================================================
    def epic_projections_function_co2(co2_scenario):        
        if region == '_am' or 'am': #Write down a way of selecting each file and merging them toegether with the right windows 
            ...
            # plot_2d_am_map(DS_y_epic_proj['yield-soy-noirr'].isel(time = -1), title = 'Original EPIC 2100')
            # plot_2d_am_map(DS_y_epic_proj['yield-soy-noirr'].isel(time = -2), title = 'Original EPIC 2099')

            DS_y_epic_proj = DS_y_epic_proj.sel(time=slice(DS_feature_proj_6mon_am_det.time[0].values, DS_feature_proj_6mon_am_det.time[-1].values))
...
def hybrid_predictions_future(DS_y_epic_proj_am, df_feature_proj_6mon2, model, rcp_scenario, hybrid_model_full, region = "_am", co2_scen = 'Default'):
    ...
    # plot_2d_am_map(DS_hybrid_proj_2['yield-soy-noirr'].mean('time'), title = 'Mean Hybrid projection')
    # plot_2d_am_map(DS_hybrid_proj_2['yield-soy-noirr'].sel(time= 2016), title = 'Hybrid prediction 2016')
    # plot_2d_am_map(DS_hybrid_proj_2['yield-soy-noirr'].isel(time= -1), title = 'Hybrid prediction final year')
...
# LOAD MODELS?
# load_dnn_model = True
# if load_dnn_model == True:
#     # Load the pipeline first:
#     full_model_hyb = joblib.load('sklearn_pipeline_AMER_r2067_loop.pkl')
#     model_hyb_test = joblib.load('sklearn_pipeline_notfull_AMER_r2067_loop.pkl')    
#     # Then, load the Keras model:
#     full_model_hyb['estimator'].model = load_model('keras_model_AMER_r2067_loop.h5')
#     model_hyb_test['estimator'].model = load_model('keras_notfull_model_AMER_r2067_loop.h5')
    
#     # Data for testing: 
#     X = pd.read_csv('dataset_input_hybrid_am_forML.csv', index_col=[0,1,2],)
#     y = pd.read_csv('dataset_obs_yield_am_forML.csv', index_col=[0,1,2],)
    
#     X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, random_state=0)
#     y_pred = model_hyb_test.predict(X_test)
#     # report performance
...
# =============================================================================# UKESM model
DS_y_epic_proj_ukesm_585_am, df_feature_proj_ukesm_585_am = projections_generation_hybrid(model = 'ukesm', rcp_scenario = 'ssp585', region = "_am", hybrid_model_full = model_to_be_used, start_date='01-01-2015', end_date='31-12-2100', co2_scen = co2_scen)
DS_y_epic_proj_ukesm_126_am, df_feature_proj_ukesm_126_am = projections_generation_hybrid(model = 'ukesm', rcp_scenario = 'ssp126', region = "_am", hybrid_model_full = model_to_be_used, start_date='01-01-2015', end_date='31-12-2100', co2_scen = co2_scen)

# GFDL model
DS_y_epic_proj_gfdl_585_am, df_feature_proj_gfdl_585_am = projections_generation_hybrid(model = 'gfdl', rcp_scenario = 'ssp585', region = "_am", hybrid_model_full = model_to_be_used, start_date='01-01-2015', end_date='31-12-2100', co2_scen = co2_scen)
DS_y_epic_proj_gfdl_126_am, df_feature_proj_gfdl_126_am = projections_generation_hybrid(model = 'gfdl', rcp_scenario = 'ssp126', region = "_am", hybrid_model_full = model_to_be_used, start_date='01-01-2015', end_date='31-12-2100', co2_scen = co2_scen)

# IPSL model
DS_y_epic_proj_ipsl_585_am, df_feature_proj_ipsl_585_am = projections_generation_hybrid(model = 'ipsl', rcp_scenario = 'ssp585', region = "_am", hybrid_model_full = model_to_be_used, start_date='01-01-2015', end_date='31-12-2100', co2_scen = co2_scen)
DS_y_epic_proj_ipsl_126_am, df_feature_proj_ipsl_126_am = projections_generation_hybrid(model = 'ipsl', rcp_scenario = 'ssp126', region = "_am", hybrid_model_full = model_to_be_used, start_date='01-01-2015', end_date='31-12-2100', co2_scen = co2_scen)
